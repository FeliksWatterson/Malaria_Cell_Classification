...
st.set_page_config(
    page_title="Hệ thống phân loại sốt rét",
    layout="wide"
)
...
st.markdown(
    "<h2 style='text-align: center; margin-bottom: 0.5rem;'>Hệ thống phân loại tế bào sốt rét</h2>",
    unsafe_allow_html=True,
)
st.markdown(
    "<p style='text-align: center; color: #555;'>Tải ảnh lam máu và hệ thống sẽ tự động phân loại tế bào.</p>",
    unsafe_allow_html=True,
)

tab_main, tab_detail, tab_info = st.tabs(
    ["Chẩn đoán", "Phân tích chi tiết", "Giới thiệu hệ thống"]
)

with tab_main:
    mode = st.radio(
        "Chế độ phân tích",
        ("Nhận diện từng ảnh", "Phân đoạn toàn bộ tiêu bản"),
        horizontal=True,
    )

    uploaded_files = st.file_uploader(
        "Ảnh đầu vào (có thể chọn nhiều ảnh)",
        type=["jpg", "png", "jpeg"],
    ...
    if uploaded_files and model is not None:
        ...
        if mode == "Nhận diện từng ảnh":
            if st.button("Chạy nhận diện cho tất cả ảnh"):
                results = []
                with st.spinner("Đang xử lý toàn bộ ảnh..."):
        ...
        elif mode == "Phân đoạn toàn bộ tiêu bản":
            if st.button("Thực hiện phân đoạn cho tất cả ảnh"):
                for uploaded_file in uploaded_files:
                    file_bytes = np.asarray(bytearray(uploaded_file.read()), dtype=np.uint8)
                    input_image = cv2.imdecode(file_bytes, 1)
                    input_image = cv2.cvtColor(input_image, cv2.COLOR_BGR2RGB)
                    ...
                    with st.container():
                        ...
                        c1.metric("Tổng số vùng phát hiện", p_count + u_count)
                        c2.metric("Số vùng nhiễm (Parasitized)", p_count)
                        c3.metric("Số vùng không nhiễm (Uninfected)", u_count)
...
with tab_detail:
    ...
    if detail_file is not None and model is not None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.metric("P(Nhiễm ký sinh trùng)", f"{infected_prob*100:.1f}%")
        with c2:
            st.metric("P(Không nhiễm)", f"{uninfected_prob*100:.1f}%")
